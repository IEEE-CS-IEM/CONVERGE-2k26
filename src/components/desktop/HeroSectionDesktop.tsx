...
const HeroSectionDesktop = () => {
  ...
  const bgUrl = crest == "redbull" ? 
  redbullPic : crest == "ferrari" ? 
  ferrariPic : crest == "mercedes" ? 
  mercedesPic : crest == "mclaren" ? 
  mclarenPic : redbullPic; 
...
>
  ...
  >
    ...
    <motion.div
      ...
      transition={{ duration: 0.6, ease: "easeOut" }}
    >
      <h2 className="text-[1rem] leading-relaxed font-formula1 text-white">
  ...
  bg-white pl-6 pb-6 rounded-bl-[2rem]"
...
>
  <div className="h-full w-full rounded-bl-[2rem] bg-black z-50 overflow-hidden">
    <Navbar />
  </div>
</motion.div>
...
>
  ...
  bg-black z-50 overflow-hidden">
    {crest === "redbull" ? (
      ...
      <img src={mercedesDriver} className="w-full h-full object-cover" />
    ) : crest === "mclaren" ? (
      ...
      <motion.div 
        className="absolute w-[5%] h-[35%] flex bottom-32 left-0 bg-white pr-6 py-6 rounded-tr-[2rem] rounded-br-[2rem]"
        initial="hidden"
        animate="visible"
        variants={{
          hidden: { opacity: 0, x: -50, y: 0, filter: "blur(10px)", scale: 0.9 },
          visible: { 
            opacity: 1, 
            x: 0, 
            y: 0, 
      ...
      >    
        <div className="h-full w-full rounded-r-[2rem] bg-black z-50 overflow-hidden">
          <SocialBar/>
        </div>
      </motion.div>
...
<motion.div
  className="absolute left-0 bottom-0 w-[30%] h-[14%] bg-white  
